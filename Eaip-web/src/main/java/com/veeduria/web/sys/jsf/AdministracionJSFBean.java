...
public class AdministracionJSFBean extends BaseJSFBean implements Serializable {
    ...
    private SegusrSFBean lookupSegusrSFBeanBean() {
        try {
            ...
            return (SegusrSFBean) c.lookup("java:global/eaipweb/SegusrSFBean!com.veeduria.sys.ejb.SegusrSFBean");
    ...
    protected AdmPuntosCtrlSFBEan lookupAdmPuntosCtrlSFBEan() {
        try {
            Context c = new InitialContext();
            return (AdmPuntosCtrlSFBEan) c.lookup("java:global/eaipweb/AdmPuntosCtrlSFBEan!com.veeduria.not.ejb.AdmPuntosCtrlSFBEan");
        } catch (NamingException ne) {
            Logger.getLogger(getClass().getName()).log(Level.SEVERE, "exception caught", ne);
            throw new RuntimeException(ne);
        }
